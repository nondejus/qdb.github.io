    ...
    protected static function query($valass, $opz){ require_once('class/Qurl.php'); $val = false;
        foreach($valass as $provider) { $provider = strtolower($provider);
            if($provider == 'ibm') { $val[$provider] = array();
                ...
                if(!isset($val[$provider]['error'])) { $out = Qurl::query('https://alchemy-language-demo.mybluemix.net/api/entities','https://alchemy-language-demo.mybluemix.net/',false,$opz['url'].'&sentiment=1&linkedData=1&relevance=1&emotion=1&subType=1');
