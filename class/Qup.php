    ...
    protected static function key($key, $val, $valass, $pr, $per, $keyper, $tmp, $ora){ $nhashpos = SYS::hashpos(Qhash($val),$per); if(!file_exists($nhashpos)) return false; $nhashposi = "$nhashpos/index.php"; $files = scandir($nhashpos);
        if(file_exists($nhashposi)) { $fi = file($nhashposi); $nhashposk = "$nhashpos/keys.php"; $nhashposl = "$nhashpos/link.php";
            if(Qin::Qdbin($key,$valass)) { global $Qdatabase; $npos = SYS::hashpos(Qhash($valass),$per); $nhashposi = "$npos/index.php"; $i = file($nhashposi);
                    ...
                    for($b = 2,$ub = count($fl); $b < $ub; $b++) {
                        if(strlen($fl[$b]) > 64) { $f = explode('/',$fl[$b]);
                            if($f[0] == 0) { $f1 = file($Qdatabase.'/'.rtrim($fl[$b]).'/keyp.php'); $g = explode('.',$f1[2]); $z = count($g) - 1;
                                ...
                                if($j > 1) { global $Qprotezione; $r5 = $npos.'/'.rtrim($g[$z]).'.php'; $f5 = file($r5);
                                    $f4 = SYS::cancella($f4,$j); $f5 = SYS::cancella($f5,$j); w($r4,$f4); w($r5,$f5);
                                    $f6 = Qdel::scrivi($npos,$ora,"\n",0,$i[2],0,0,1);
                                    w($Qdatabase.'/'.rtrim($fl[$b]).'/keyp.php',$Qprotezione.$f[1].'.'.$f6."\n");
                                    $npos .= '/in.php'; $g = r($npos); $g = explode('.',$g); $g[1]--; $g[2]--;
                                    w($npos,$g[0].'.'.$g[1].'.'.$g[2].'.'.rtrim($g[3])."\n");
                                }
                ...
                } else { Qdel::elimina($keyper,$fi[2]); Qdel::directory($nhashpos); }
                for($a = count($fi); $a > 2; $a--) $fi[$a] = $fi[$a - 1];
                $keyper = $tmp.'/'.substr($fi[2],0,3).'/'.substr($fi[2],3,$fi[2][12]);
                $keyv = "$keyper/keyv.php"; $keyn = "$keyper/keyn.php"; Qdel::protezione($keyv,$keyn);
                a($keyv,$fi[2]); a($keyn,$i[2]); $fi[2] = $i[2]; w($nhashposi,$fi); return true;
            }
        } return false;
    }
}