...
class Qer extends Qdel
{
    ...
    private static function delink($keyass, $key) { global $Qdatabase; $fp = file("$Qdatabase/link.php"); $link = rtrim(SYS::combina($keyass,1)).'#'.rtrim(SYS::combina($key,1)).".0\n"; $j = array_search($link,$fp); if($j > 1) { $fp = SYS::cancella($fp,$j); w("$Qdatabase/link.php",$fp); }}
    private static function elimina_adesso($file, $per, $ora, $opz=false) { $es = false; $file = file($file); for($a=2, $u=count($file); $a<$u; $a++) { $es = Qdel::elimina($per, '#'.rtrim($file[$a]), 1, $ora); if($es) break; if($opz){ $es = Qdel::elimina($p, rtrim($m[$a]), 1, $ora); if($es) break; }} return $es; }
    private static function delete($fp, $fl, $x, $p, $pr, $keypery, $keyperl, $hashpos, $ora=0, $type=0) { global $Qdatabase; global $Qlivtime; if(count($fp) == 3) { unlink($keypery); unlink($keyperl); } else { $fp = SYS::cancella($fp,$x); $fl = SYS::cancella($fl,$x); w($keypery,$fp); w($keyperl,$fl); } $f1 = file("$hashpos/index.php"); $f2 = file("$p/keyp.php"); $f = explode('.', $f2[2]); $x = count($f)-1; $r4 = $Qdatabase.'/'; for($a=0; $a<$x; $a++) $r4 .= $f[$a].'/'; $r = $pr.'0'; $rf = rtrim($f[$x]); $r5 = "$r4$r/v$rf.php";
        if(file_exists($r5)) { $f5 = file($r5); $j = array_search($f1[2], $f5); if($j > 1) { $f4 = file("$r4$r/$rf.php"); $f5 = SYS::cancella($f5,$j); $f4 = SYS::cancella($f4,$j); w($r5,$f5); w("$r4$r/$rf.php",$f4); if($ora) { $x = substr($p,strlen($Qdatabase)+3); $f = explode('/', $x); $pr = $ora.'-'.$f[0].'.'; for($a=1, $u=count($f)-$Qlivtime-1; $a<$u; $a++) $pr .= $f[$a].'.'; Qcopy("$p/log.php",$hashpos.'/'.$pr.'0.php'); } Qdel::elimina($r4.$r, $rf, 2); if($type != 888) Qdel::directory($p); }}
    }
