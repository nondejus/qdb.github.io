...
function Qsync($val, $opz=0){ if($opz) $f = fopen(QFILESYNC,'w+'); else { $f = fopen(QFILESYNC,'a+'); if(file_exists($val) && (time()-filemtime($val)) > 0) copy($val, substr($val,0,-4).'_SYNC_.php'); } fwrite($f, $val."\n"); fclose($f); }
