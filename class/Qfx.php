...
function a($file=false, $val=false){ if($file && $val){ Qsync($file); if(is_array($val)) $val = implode('',$val); $f = fopen($file,'a+'); if($f){ umask(0); fwrite($f,$val); fclose($f); return true; }} return false; }
function w($file=false, $val=false, $opz=false){ if($file && $val){ Qsync($file); if(is_array($val)) $val = implode('',$val); if(!$opz) $f = fopen($file,'w+'); if($f){ umask(0); fwrite($f,$val); fclose($f); return true; }} return false; }
...
function Qsync($val, $opz=0){ if($opz) { global $Qprotezione; $val = $Qprotezione.$val; $f = fopen(QFILESYNC,'w+'); } else { $f = fopen(QFILESYNC,'a+'); umask(0); Qcopy($val, substr($val,0,-4).'_SYNC_.php'); } fwrite($f, $val."\n"); fclose($f); }
