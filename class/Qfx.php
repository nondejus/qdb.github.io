...
function Qsync($val, $opz=0){ if($opz) { global $Qprotezione; $val = $Qprotezione.$val; $f = fopen(QFILESYNC,'w+'); } else { $f = fopen(QFILESYNC,'a+'); Qcopy($val, substr($val,0,-4).'_SYNC_.php'); } fwrite($f, $val."\n"); fclose($f); }
