...
class DB
{
    ...
    public static function in($k=null, $v=null, $s=null, $o=0){ if(DB::exit($k,$v,$s,$o)) return null; global $Qdatabase; $x = fopen("$Qdatabase/lock.php","w"); if(flock($x,LOCK_EX) == false) { fclose($x); return null; } else { require_once 'class/Qin.php'; $v = Qin::Qdbin($k,$v,$s,$o); Qsync('*'); flock($x,LOCK_UN); fclose($x); return $v; }}
    public static function ver($k=null, $v=null, $s=null, $o=null){ if(DB::exit($k,$v,$s,$o)) return null; global $Qdatabase; $x = fopen("$Qdatabase/lock.php","w"); if(flock($x,LOCK_EX) == false) { fclose($x); return null; } else { require_once 'class/Qver.php'; $v = Qver::Qdbver($k,$v,$s,$o); Qsync('*'); flock($x,LOCK_UN); fclose($x); return $v; }}
