        ...
        if($par != $arr) {
            ...
            for($a=2; $a<count($keybase); $a++) {
                if(rtrim($keybase[$a]) == $key) { $keypar = Qchiaro(file($par)); $p = 2; $b1 = 0; $b2 = 0; $ok = false; for($b=2; $b<count($keypar); $b++) { $str = explode(".", $keypar[$b]); if($a == $str[0]) { if($pos == 3) { $keypar[$b] = substr($keypar[$b],0,-2)."1\n"; Qw($par, Qscuro($keypar), true); Qsync('*'); exit('1'); } elseif($pos == 4) { $keypar[$b] = substr($keypar[$b],0,-2)."0\n"; Qw($par, Qscuro($keypar), true); Qsync('*'); exit('1'); } else { $kb1[$b1] = $keypar[$b]; $b1++; $ok = true; }} else { if($ok == false) { $kcb = $str[0]; $p = $b + 1; } else break; }} 
                    if($pos == 1) { $str = explode(".", $keypar[$b]); $kcb = $str[0]; } elseif($pos == 2) { for($c=($p+$b1); $c<count($keypar); $c++) $keypar[$c-$b1] = $keypar[$c]; Qw($par, $Qprotezione); for($d=2; $d<(count($keypar)-$b1); $d++) Qa($par, Qscuro($keypar[$d]), true); Qsync('*'); exit('1'); } for($b=2; $b<count($keypar); $b++) { $str = explode(".", $keypar[$b]); if($kcb == $str[0]) { $kb2[$b2] = $keypar[$b]; $b2++; }}
                    if($pos == 1) { for($b=$p; $b<($p+$b2); $b++) $keypar[$b] = $kb2[$b-$p]; for($b=($p+$b2); $b<($p+$b2+$b1); $b++) $keypar[$b] = $kb1[$b-$p-$b2]; } else { $p -= $b2; for($b=$p; $b<($p+$b1); $b++) $keypar[$b] = $kb1[$b-$p]; for($b=($p+$b1); $b<($p+$b1+$b2); $b++) $keypar[$b] = $kb2[$b-$p-$b1]; } Qw($par, Qscuro($keypar), true); Qsync('*'); exit('1');
                }
            } exit('9');
        }
    }
