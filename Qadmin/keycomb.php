<?php
    ...
    require_once 'cookie.php'; $Qkeybase = file("$Qdatabase/key.php"); $l = file("$Qdatabase/link.php"); $l1 = array(); $l2 = array(); $l3 = array(); $inv = array(); $val = array(); $link = array(); $y = array(); $x = false;
    ...
    if($par == 0) {
        if($arr == 0) { $note = Qchiaro(file("$dirschema/note.php")); $nval = array("\n","\n");
            for($a=2, $ua=count($note); $a<$ua; $a++) { $z = explode('=',$note[$a],2); $x = explode('#',$z[0]);
                $y = explode('.',$x[0]); $x[0] = rtrim($Qkeybase[$y[0]]); for($b=1, $ub=count($y); $b<$ub; $b++) $x[0] .= '.'.rtrim($Qkeybase[$y[$b]]);
                $y = explode('.',$x[2]); $x[2] = rtrim($Qkeybase[$y[0]]); for($b=1, $ub=count($y); $b<$ub; $b++) $x[2] .= '.'.rtrim($Qkeybase[$y[$b]]);
                    ...
                    for($c=2; $c<$lun; $c++) { $si = true; $lk = array(0); $nu = array(0); $k = 0; $fc[0] = '0.0.0'; $pc = substr($keycomb[$c], 0, -3); $w = explode('.',$keycomb[$c]); 
                        ...
                        $kcomb = str_replace('.', '/', $pc).'/in.php'; $kcomb = $Qdatabase.'/'.$kcomb;
                            if(file_exists($kcomb)){ $fc = file($kcomb); $str = explode('.', $pc); $pc = '';
                                if($ty != 1) { for($j=0, $u=count($str); $j<$u; $j++) $pc .= $Qkeybase[$str[$j]].'.'; $pc = substr($pc, 0, -1).'#0.0';
                        ...
                        if($si) { if(isset($Qkeybase[$str[0]])) tabella($a, $d, $Qkeybase[$str[0]], $str, $Qkeybase, $st, $fc[0], $lk, $nu, $inv, $note, $nval); } $tmp = $str[0];
                    }
                }
            } exit(json_encode($val));
        } else { for($a=2, $ua=count($Qkeybase); $a<$ua; $a++) { if(rtrim($Qkeybase[$a]) == $key) { for($x=1; $x<$pos; $x++) { $keycomb = Qchiaro(file("$dirschema/$x.php")); for($c=2, $uc=count($keycomb); $c<$uc; $c++) { $str = explode('.', $keycomb[$c]); if(rtrim($Qkeybase[$str[0]]) == $key) exit('7'); }} Qa("$dirschema/1.php", Qscuro("$a.0\n"), true); Qsync('*'); exit('1'); }} exit('8'); }
    } else { $par = "$dirschema/$par.php"; $arr = "$dirschema/$arr.php"; if($par != $arr) { for($a=2, $ua=count($Qkeybase); $a<$ua; $a++) { if(rtrim($Qkeybase[$a]) == $key) { $keypar = Qchiaro(file($par)); $keyarr = Qchiaro(file($arr)); $k = count($keyarr); $p = 0; for($b=2, $ub=count($keypar); $b<$ub; $b++) { $str = explode('.', $keypar[$b]); if($a == $str[0]) { $keyarr[$k] = $keypar[$b]; $k++; } else { $kpar[$p] = $keypar[$b]; $p++; }} Qw($par, $Qprotezione); for($d=0; $d<$p; $d++) Qa($par, Qscuro($kpar[$d]), true); Qw($arr, Qscuro($keyarr), true); Qsync('*'); exit('1'); }} exit('9');
        } else { for($a=2, $ua=count($Qkeybase); $a<$ua; $a++) { if(rtrim($Qkeybase[$a]) == $key) { $keypar = Qchiaro(file($par)); $p = 2; $b1 = 0; $b2 = 0; $ok = false; for($b=2, $ub=count($keypar); $b<$ub; $b++) { $str = explode(".", $keypar[$b]); if($a == $str[0]) { if($pos == 3) { $keypar[$b] = substr($keypar[$b],0,-2)."1\n"; Qw($par, Qscuro($keypar), true); Qsync('*'); exit('1'); } elseif($pos == 4) { $keypar[$b] = substr($keypar[$b],0,-2)."0\n"; Qw($par, Qscuro($keypar), true); Qsync('*'); exit('1'); } else { $kb1[$b1] = $keypar[$b]; $b1++; $ok = true; }} else { if($ok == false) { $kcb = $str[0]; $p = $b + 1; } else break; }} 
