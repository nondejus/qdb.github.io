<?php
    ...
    require_once('cookie.php'); require_once('function.php'); if($dbtype[3] == "test\n") $Qdatabase .= '-test'; elseif($dbtype[3] == "clone\n") $Qdatabase .= '-clone'; $keybase = file("$Qdatabase/key.php"); $l = file("$Qdatabase/link.php"); $l1 = array(); $l2 = array(); $l3 = array(); $inv = array(); $val = array(); $link = array(); $y = array(); $x = false;
    for($a=2; $a<count($l); $a++) { $s = explode('#',$l[$a]); if($s[1] == $x) $l3[] = $l[$a]; else { $x = false; if(strpos($s[1],".0\n")) { if(strpos($s[0],'@')) { $s[0] = substr($s[0],0,-2); $l3[] = $s[0].'#'.rtrim($s[1]).".@\n"; } else { if(isset($l[$a+1])) { if(strpos($l[$a+1],'#'.$s[0].".0\n")) { $l2[] = $l[$a]; $a++; $l2[] = $l[$a]; } else $l2[] = $l[$a]; } else $l2[] = $l[$a]; }} elseif(strpos($s[1],".1\n")) { $l3[] = $l[$a]; $x = substr($s[1],0,-2).$s[0].".0\n"; $y[] = rtrim($x); } else $l1[] = $l[$a]; }} $link = array_merge($l1,$l2,$l3);
    ...
    function tabella($arr, $lun, $key, $keyc, $keyb, $st, $fc, $lk, $nu, $inv, $note, $nval) { global $dirschema; global $val; $arr--; $lun--; $st--; $f = explode('.', $fc); $val[$arr][$lun]['value'] = rtrim($key);
        if(is_array($lk)) { $val[$arr][$lun]['block'][$st] = array('T' => (int)$f[2]); $k = count($keyc)-1; $msg = keyb($keyb,$k,$keyc); $val[$arr][$lun]['block'][$st]['K'] = $msg; if($keyc[$k] == "x\n") $val[$arr][$lun]['block'][$st]['close'] = true;
            if($lk[0]) {
                for($b=0; $b<count($lk); $b++) { $str = explode('#', $lk[$b]); $s2 = explode('.', $str[1]); $k = count($s2)-1; $msg = keyb($keyb,$k,$s2);
                    if($s2[$k] == 0) { 
    ...
    if($par == 0) {
        if($arr == 0) { $note = Qchiaro(file("$dirschema/note.php")); $nval = array("\n","\n");
            for($a=2; $a<count($note); $a++) { $z = explode('=',$note[$a],2); $x = explode('#',$z[0]);
                $y = explode('.',$x[0]); $x[0] = rtrim($keybase[$y[0]]); for($b=1; $b<count($y); $b++) $x[0] .= '.'.rtrim($keybase[$y[$b]]);
                $y = explode('.',$x[2]); $x[2] = rtrim($keybase[$y[0]]); for($b=1; $b<count($y); $b++) $x[2] .= '.'.rtrim($keybase[$y[$b]]);
                    ...
                    for($c=2; $c<$lun; $c++) { $si = true; $lk = array(0); $nu = array(0); $k = 0; $fc[0] = '0.0.0'; $pc = substr($keycomb[$c], 0, -3); $w = explode('.',$keycomb[$c]); 
                        ...
                        $kcomb = str_replace('.', '/', $pc).'/in.php'; $kcomb = $Qdatabase.'/'.$kcomb;
                            if(file_exists($kcomb)){ $fc = file($kcomb); $str = explode('.', $pc); $pc = '';
                                if($ty != 1) { for($j=0; $j<count($str); $j++) $pc .= $keybase[$str[$j]].'.'; $pc = substr($pc, 0, -1).'#0.0';
                                    for($j=0; $j<count($link); $j++) { $lnk = explode('#', $link[$j]); $ln = $lnk[0].".0\n"; $r = false;
        ...
        } else { for($a=2; $a<count($keybase); $a++) { if(rtrim($keybase[$a]) == $key) { for($x=1; $x<$pos; $x++) { $keycomb = Qchiaro(file("$dirschema/$x.php")); for($c=2; $c<count($keycomb); $c++) { $str = explode('.', $keycomb[$c]); if($keybase[$str[0]] == $key) exit('7'); }} $fp = fopen("$dirschema/1.php",'a'); fwrite($fp, Qscuro("$a.0\n")); fclose($fp); exit('1'); }} exit('8'); }
    } else { $par = "$dirschema/$par.php"; $arr = "$dirschema/$arr.php";
        if($par != $arr) {
            for($a=2; $a<count($keybase); $a++) { if(rtrim($keybase[$a]) == $key) { $keypar = Qchiaro(file($par)); $keyarr = Qchiaro(file($arr)); $k = count($keyarr); $p = 0; for($b=2; $b<count($keypar); $b++) { $str = explode('.', $keypar[$b]); if($a == $str[0]) { $keyarr[$k] = $keypar[$b]; $k++; } else { $kpar[$p] = $keypar[$b]; $p++; }} $fp = fopen($par, 'w'); fwrite($fp, $Qprotezione); for($d=0; $d<$p; $d++) fwrite($fp, Qscuro($kpar[$d])); fclose($fp); Qw($arr,Qscuro($keyarr)); exit('1'); }} exit('9');
        } else {
            for($a=2; $a<count($keybase); $a++) {
                if(rtrim($keybase[$a]) == $key) { $keypar = Qchiaro(file($par)); $p = 2; $b1 = 0; $b2 = 0; $ok = false; for($b=2; $b<count($keypar); $b++) { $str = explode(".", $keypar[$b]); if($a == $str[0]) { if($pos == 3) { $keypar[$b] = substr($keypar[$b],0,-2)."1\n"; Qw($par,Qscuro($keypar)); exit('1'); } elseif($pos == 4) { $keypar[$b] = substr($keypar[$b],0,-2)."0\n"; Qw($par,Qscuro($keypar)); exit('1'); } else { $kb1[$b1] = $keypar[$b]; $b1++; $ok = true; }} else { if($ok == false) { $kcb = $str[0]; $p = $b + 1; } else break; }} 
                    if($pos == 1) { $str = explode(".", $keypar[$b]); $kcb = $str[0]; } elseif($pos == 2) { for($c=($p+$b1); $c<count($keypar); $c++) $keypar[$c-$b1] = $keypar[$c]; $fp = fopen($par, 'w'); fwrite($fp, $Qprotezione); for($d=2; $d<(count($keypar)-$b1); $d++) fwrite($fp, Qscuro($keypar[$d])); fclose($fp); exit('1'); } for($b=2; $b<count($keypar); $b++) { $str = explode(".", $keypar[$b]); if($kcb == $str[0]) { $kb2[$b2] = $keypar[$b]; $b2++; }}
                    if($pos == 1) { for($b=$p; $b<($p+$b2); $b++) $keypar[$b] = $kb2[$b-$p]; for($b=($p+$b2); $b<($p+$b2+$b1); $b++) $keypar[$b] = $kb1[$b-$p-$b2]; } else { $p -= $b2; for($b=$p; $b<($p+$b1); $b++) $keypar[$b] = $kb1[$b-$p]; for($b=($p+$b1); $b<($p+$b1+$b2); $b++) $keypar[$b] = $kb2[$b-$p-$b1]; } Qw($par,Qscuro($keypar)); exit('1');
                }
            } exit('9');
        }
    }
