<?php
    ...
    } elseif($type == 11){ $fp = file("$dirschema/command.php"); $val = str_replace('#####1',"'",$val); $val = str_replace('#####2','"',$val); $fp[$pos] = $val."\n"; if($dbtype[3] == "test\n") $tmp = '.test'; elseif($dbtype[3] == "clone\n") $tmp = '.clone'; else $tmp = ''; $str = '<?php require "../../Quantico'.$tmp.'.php"; use Quantico as Q; $val = '.$val.'; echo "<pre>"; print_r($val); echo "</pre>"; ?>'; $cmd = '<?php $val = '.$val.'; ?>'; Qw('temp/risultato.php', $str); Qw('temp/valore.php', $cmd); Qw("$dirschema/command.php", $fp, true); exit('OK');
    ...
    } elseif($type == 15){ $ok = true; $per = "$dirschema/note.php"; $pos = keypos($pos); $key = file($per); $fp = Qchiaro($key); for($a=2; $a<count($fp); $a++) { $x = explode('=',$fp[$a]); if($pos == $x[0]) { $key[$a] = Qscuro("$pos=$val"); $ok = false; break; }} if($ok) Qa($per, Qscuro("$pos=$val"), true); else Qw($per, $key, true); exit('OK');
    } elseif($type == 16){ $ok = true; $per = "$dirschema/note.php"; $pos = keypos($pos); $key = file($per); $fp = Qchiaro($key); for($a=2; $a<count($fp); $a++) { $x = explode('=',$fp[$a]); if($pos == $x[0]) { $key = Qcancella($key,$a); $ok = false; break; }} if($ok) exit('24'); else { Qw($per, $key, true); exit('OK'); }
