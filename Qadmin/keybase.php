<?php
            ...
            if($val == rtrim($keybase[$a])) { $cestino[$a] = $keybase[$a]; $keybase[$a] = "\n"; Qw("$Qdatabase/key.php", $keybase, true); Qw("$Qdatabase/trash.php", $cestino, true); $tmp = $a; $key = file("$dirschema/key.php"); $pos++; $x = explode(' ',rtrim($key[$pos])); $j = array_search($a,$x); 
                if($j !== false) { $x = Qcancella($x,$j); $key[$pos] = ''; if($x) { $y = count($x)-1; for($a=0; $a<$y; $a++) $key[$pos] .= $x[$a].' '; $key[$pos] .= $x[$y]; } $key[$pos] .= "\n";
                    for($b=1; $b<$old; $b++) { $keycomb = Qchiaro(file("$dirschema/$b.php")); $k = 0;
                        ...
                        if($k) { Qw("$dirschema/$b.php",$Qprotezione); for($d=0; $d<$k; $d++) if($kct[$d]) Qa("$dirschema/$b.php", Qscuro($kct[$d]), true); }
                    } exit(keybase($key));
                }
            }
        } exit('0');
    ...
    } elseif($type == 11){ $fp = file("$dirschema/command.php"); $val = str_replace('#####1',"'",$val); $val = str_replace('#####2','"',$val); $fp[$pos] = $val."\n"; if($dbtype[3] == "test\n") $tmp = '.test'; elseif($dbtype[3] == "clone\n") $tmp = '.clone'; else $tmp = ''; $str = '<?php require "../../Quantico'.$tmp.'.php"; use Quantico as Q; $val = '.$val.'; echo "<pre>"; print_r($val); echo "</pre>"; ?>'; $cmd = '<?php $val = '.$val.'; ?>'; Qw('temp/risultato.php', $str); Qw('temp/valore.php', $cmd); Qw("$dirschema/command.php", $fp, true); Qsync('*'); exit('OK');
    } elseif($type == 12){ $keyc = "'".str_replace(" ", "','", rtrim($pos))."'"; if($dbtype[3] == "test\n") $tmp = '.test'; elseif($dbtype[3] == "clone\n") $tmp = '.clone'; else $tmp = ''; $str = '<?php require "../../Quantico'.$tmp.'.php"; $val = Q\DB::out(\''.$old.'\',array('.$keyc.'),\''.$val.'\'); echo "<pre>"; print_r($val); echo "</pre>"; ?>'; Qw('temp/risultato.php', $str); exit('1');
