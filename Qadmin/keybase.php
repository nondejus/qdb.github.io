<?php
    ...
    } elseif($type == 11){ $fp = file("$dirschema/command.php"); $val = str_replace('#####1',"'",$val); $val = str_replace('#####2','"',$val); $fp[$pos] = $val."\n"; if($dbtype[3] == "test\n") $tmp = '.test'; elseif($dbtype[3] == "clone\n") $tmp = '.clone'; else $tmp = ''; $str = '<?php require "../../Quantico'.$tmp.'.php"; use Quantico as Q; $val = '.$val.'; echo "<pre>"; print_r($val); echo "</pre>"; ?>'; $cmd = '<?php $val = '.$val.'; ?>'; Qw('temp/risultato.php', $str); Qw('temp/valore.php', $cmd); Qw("$dirschema/command.php", $fp); exit('OK');
