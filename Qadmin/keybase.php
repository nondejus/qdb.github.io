<?php
    ...
    require_once 'cookie.php'; $keybase = file("$Qdatabase/key.php"); $cestino = file("$Qdatabase/trash.php"); $lkb = count($keybase); $msg='';
    ...
    function keybase($key=false){ global $dirschema; if($key) { global $Qdatabase; $keybase = file("$Qdatabase/key.php"); Qw("$dirschema/key.php", $key, true); } else { global $keybase; $key = file("$dirschema/key.php"); }
        for($a=2; $a<count($key); $a++) { $y = rtrim($key[$a]); if($y) { $x = explode(' ',$y); for($b=0; $b<count($x); $b++) { if(isset($keybase[$x[$b]])) $msg[$a-2][$b] = rtrim($keybase[$x[$b]]); else $msg[$a-2][$b] = ''; }} else $msg[$a-2] = array(); } Qsync('*'); return json_encode($msg); 
    }
    ...
    if($type == 1){ // --------------------------------------------------------------------------------------------------------------------------------------- JSON ---> Costruzione del menu ( KEY Base - 7 parti )
        ...
        for($a=2; $a<$lkb; $a++) {
            if($val == rtrim($keybase[$a])) { $cestino[$a] = $keybase[$a]; $keybase[$a] = "\n"; Qw("$Qdatabase/key.php", $keybase, true); Qw("$Qdatabase/trash.php", $cestino, true); $tmp = $a; $key = file("$dirschema/key.php"); $pos++; $x = explode(' ',rtrim($key[$pos])); $j = array_search($a,$x); 
                if($j !== false) { $x = Qcancella($x,$j); $key[$pos] = ''; if($x) { $y = count($x)-1; for($a=0; $a<$y; $a++) $key[$pos] .= $x[$a].' '; $key[$pos] .= $x[$y]; } $key[$pos] .= "\n"; for($b=1; $b<$old; $b++) { $keycomb = Qchiaro(file("$dirschema/$b.php")); $k = 0; for($c=2; $c<count($keycomb); $c++) { $str = explode('.', $keycomb[$c]); $kct[$k] = ''; for($s=0; $s<count($str)-1; $s++) if($tmp != $str[$s]) $kct[$k] .= $str[$s].'.'; else break; if($kct[$k] != '') $kct[$k] .= "0\n"; $k++; } if($k) { Qw("$dirschema/$b.php",$Qprotezione); for($d=0; $d<$k; $d++) if($kct[$d]) Qa("$dirschema/$b.php", Qscuro($kct[$d]), true); }} exit(keybase($key)); }
            }
        } exit('0');
    ...
    } elseif($type == 8){ if($val == rtrim($cestino[$old])) { $keybase[$old] = $cestino[$old]; $cestino[$old] = "\n"; Qw("$Qdatabase/key.php", $keybase, true); Qw("$Qdatabase/trash.php", $cestino, true); $key = file("$dirschema/key.php"); $key[2] = rtrim($key[2]); if($key[2]) $key[2] .= ' '.$old."\n"; else $key[2] = $old."\n"; exit(keybase($key)); } else exit('10');
    ...
    } elseif($type == 11){ $fp = file("$dirschema/command.php"); $val = str_replace('#####1',"'",$val); $val = str_replace('#####2','"',$val); $val = str_replace('Q\\','Quantico\\',$val); $fp[$pos] = $val."\n"; if($dbtype[3] == "test\n") $tmp = '.test'; elseif($dbtype[3] == "clone\n") $tmp = '.clone'; else $tmp = ''; $str = '<?php require "../../Quantico'.$tmp.'.php"; $val = '.$val.'; echo "<pre>"; print_r($val); echo "</pre>"; ?>'; $cmd = '<?php $val = '.$val.'; ?>'; Qw('temp/risultato.php', $str); Qw('temp/valore.php', $cmd); Qw("$dirschema/command.php", $fp, true); Qsync('*'); exit('OK');
    } elseif($type == 12){ $keyc = "'".str_replace(" ", "','", rtrim($pos))."'"; if($dbtype[3] == "test\n") $tmp = '.test'; elseif($dbtype[3] == "clone\n") $tmp = '.clone'; else $tmp = ''; $str = '<?php require "../../Quantico'.$tmp.'.php"; $val = Quantico\DB::out(\''.$old.'\',array('.$keyc.'),\''.$val.'\'); echo "<pre>"; print_r($val); echo "</pre>"; ?>'; Qw('temp/risultato.php', $str); exit('1');
    } elseif($type == 13){ $k = explode('|',substr($pos,0,-1)); $v = explode(' ',rtrim($val)); if($dbtype[3] == "test\n") $tmp = '.test'; elseif($dbtype[3] == "clone\n") $tmp = '.clone'; else $tmp = ''; $str = '<?php require "../../Quantico'.$tmp.'.php"; $val = Quantico\DB::out(\''.$old.'\',array('; $b = -1; foreach($k as $a) { $b++; if(strpos($a, ',')) { $a = "'".str_replace(",", "','", $a)."'"; $str .= "'$v[$b]' => array($a),"; } else $str .= "'$v[$b]' => '$a'"; } $str .= ')); echo "<pre>"; print_r($val); echo "</pre>"; ?>'; Qw('temp/risultato.php', $str); exit('1');
    ...
    } elseif($type == 15){ $ok = true; $per = "$dirschema/note.php"; $pos = keypos($pos); $key = file($per); $fp = Qchiaro($key); for($a=2; $a<count($fp); $a++) { $x = explode('=',$fp[$a]); if($pos == $x[0]) { $key[$a] = Qscuro("$pos=$val"); $ok = false; break; }} if($ok) Qa($per, Qscuro("$pos=$val"), true); else Qw($per, $key, true); Qsync('*'); exit('OK');
    } elseif($type == 16){ $ok = true; $per = "$dirschema/note.php"; $pos = keypos($pos); $key = file($per); $fp = Qchiaro($key); for($a=2; $a<count($fp); $a++) { $x = explode('=',$fp[$a]); if($pos == $x[0]) { $key = Qcancella($key,$a); $ok = false; break; }} if($ok) exit('24'); else { Qw($per, $key, true); Qsync('*'); exit('OK'); }
