<?php
    ...
    } elseif($type == 11){ $vietati = ['eval','function','system','shell','exec']; $fp = explode('(',$val); foreach($fp as $cmd) foreach($vietati as $str) if(strpos($cmd,$str) !== false) exit('0'); $val = str_replace('#####1',"'",$val); $val = str_replace('#####2','"',$val); $fp = file("$dirschema/command.php"); $fp[$pos] = $val."\n"; $val = str_replace('Q\\','Quantico\\',$val); $str = '<?php require "../../Quantico'.$tmp.'.php"; $val = '.$val.'; echo "<pre>"; print_r($val); echo "</pre>"; ?>'; $cmd = '<?php $val = '.$val.'; ?>'; Qw('temp/risultato.php', $str); Qw('temp/valore.php', $cmd); Qw("$dirschema/command.php", $fp, true); Qsync('*'); exit('OK');
