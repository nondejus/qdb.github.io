<?php
    ...
    function keybase($key=false){ global $dirschema; if($key) { global $Qdatabase; $keybase = file("$Qdatabase/key.php"); Qw("$dirschema/key.php", $key, true); } else { global $keybase; $key = file("$dirschema/key.php"); }
        for($a=2, $ua=count($key); $a<$ua; $a++) { $y = rtrim($key[$a]); if($y) { $x = explode(' ',$y); for($b=0, $ub=count($x); $b<$ub; $b++) { if(isset($keybase[$x[$b]])) $msg[$a-2][$b] = rtrim($keybase[$x[$b]]); else $msg[$a-2][$b] = ''; }} else $msg[$a-2] = array(); } Qsync('*'); return json_encode($msg); 
    }
