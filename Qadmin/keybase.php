<?php
    ...
    } elseif($type == 11){ $val = json_decode($val); if(strpos($val, '"') !== false && strpos($val, "'") !== false) exit('<h1>-:|:- Prohibited -:|:-</h1>'); if(strpos($val, 'eval') !== false && strpos($val, '(') !== false && strpos($val, ')') !== false) exit('<h1>-:|:- Prohibited -:|:-</h1>'); if(strpos($val, 'system') !== false && strpos($val, '(') !== false && strpos($val, ')') !== false) exit('<h1>-:|:- Prohibited -:|:-</h1>');
        $fp = file("$dirschema/command.php"); $fp[$pos] = $val."\n"; $val = str_replace('Q\\','Quantico\\',$val); $str = '<?php require "../../Quantico'.$tmp.'.php"; $val = '.$val.'; echo "<pre>"; print_r($val); echo "</pre>"; ?>'; $cmd = '<?php $val = '.$val.'; ?>'; Qw('temp/risultato.php', $str); Qw('temp/valore.php', $cmd); Qw("$dirschema/command.php", $fp, true); Qsync('*'); exit('OK');
