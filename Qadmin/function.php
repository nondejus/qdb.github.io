<?php 
    ...
    function Qscuro($keycomb, $key_user=false, $key_pass=false){ $key = substr(Qrawdec($_POST['sid']),0,64); if($key_user) $key = $key_user.substr($key,32); if($key_pass) $key = substr($key,0,32).$key_pass; $iv = substr($key,16,32); if(is_array($keycomb)) { for($a=2, $u=count($keycomb); $a<$u; $a++) $keycomb[$a] = Qcrypt(Qh($keycomb[$a]).$keycomb[$a], $key, $iv)."\n"; return $keycomb; } else return Qcrypt(Qh($keycomb).$keycomb, $key, $iv)."\n"; }
    function Qchiaro($keycomb){ $key = substr(Qrawdec($_POST['sid']),0,64); $iv = substr($key,16,32); if(is_array($keycomb)) { for($a=2, $u=count($keycomb); $a<$u; $a++) { $r = Qdecrypt(rtrim($keycomb[$a]), $key, $iv); if($r && substr($r,0,128) == Qh(substr($r,128))) $keycomb[$a] = substr($r,128); } return $keycomb; } else { $r = Qdecrypt($keycomb, $key, $iv); if($r && substr($r,0,128) == Qh(substr($r,128))) return substr($r,128); else return false; }}
