...
require_once 'cookie.php'; $tmp = ''; if($dbtype[3] == "test\n") $tmp = '.test'; elseif($dbtype[3] == "clone\n") $tmp = '.clone'; require_once '../Quantico'.$tmp.'.php'; $ke = explode('@',$key);
...
function Qdettagli($dati){ if(is_array($dati)) { if(isset($dati['K'])) { $msg = array(); // JSON ---> Costruzione del Qdb::out() completo della KEY Primaria
...
if($type == 1) { $dati = Qdb::out("$ke[0]", rtrim($val), -1); exit(Qdettagli($dati)); }
elseif($type == 2) { if(Qdb::in($key, rtrim($val), rtrim($pos)) !== false) exit('OK'); }
elseif($type == 3) { if(Qdb::del($key, rtrim($pos), rtrim($val)) !== false) exit('OK'); }
elseif($type == 4) { $dati = Qdb::out($key, rtrim($val)); $kc = explode('.', $key); $val = array();
...
elseif($type == 6) { $dati = Qdb::out($val, -1); exit(Qdettagli($dati)); }
else { $lng = file('language/'.rtrim($dbtype[4]).'/keyview.php', FILE_IGNORE_NEW_LINES);
    if($type == 7) { $dati = Qdb::out("-$ke[0]", $val, -1);
    ...
    } elseif($type == 8) { $key = base64_decode($key); $dati = Qdb::out($key,base64_decode($val),-1); $e1 = strpos($key,':')+1; $e2 = strpos($key,')'); $max = substr($key,$e1,$e2-$e1);
    ...
    } elseif($type == 9) { $key = base64_decode($key); $dati = Qdb::out($key,base64_decode($val),-1); $e1 = strpos($key,':')+1; $e2 = strpos($key,')'); $e3 = strpos($key,'(')+1; $e4 = strpos($key,'@'); $e5 = substr($key,$e4,$e3-$e4-1); $max = substr($key,$e1,$e2-$e1);
